...
RUN \
	cd /tmp && \
	curl 'https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-1.5.0.tar.gz' > elasticsearch-1.5.0.tar.gz && \
	tar xvzf elasticsearch-1.5.0.tar.gz && \
	rm -f elasticsearch-1.5.0.tar.gz && \
	mv /tmp/elasticsearch-1.5.0 /usr/local/elasticsearch
