...
curl -s -XPUT "http://${ES_SERVICE_NAME}:9200/_cluster/settings" \
	-d "{ \"transient\" : { \"cluster.routing.allocation.exclude._ip\" : \"$LAST_ES_IP\" } }"
...
# Get service UUID
ES_SERVICE_UUID=`\
	curl -s -H "Authorization: $DOCKERCLOUD_AUTH" -H "Accept: application/json" \
		${DOCKERCLOUD_REST_HOST}/api/app/v1/${ORG_NAME}service/?limit=250 |\
...
curl -s -H "Content-Type: application/json" -H "Authorization: $DOCKERCLOUD_AUTH" -H "Accept: application/json" \
	${DOCKERCLOUD_REST_HOST}/api/app/v1/${ORG_NAME}service/${ES_SERVICE_UUID}/ \
	-XPATCH \
	-d "{ \"target_num_containers\" : $TARGET_NUM_CONTAINERS }"
curl -s -H "Authorization: $DOCKERCLOUD_AUTH" -H "Accept: application/json" \
	${DOCKERCLOUD_REST_HOST}/api/app/v1/${ORG_NAME}service/${ES_SERVICE_UUID}/scale/ \
	-XPOST
